...
def supplier_list():
    ...
    # Import form locally to avoid circular imports
    try:
    ...
    try:
        ...
        # NEW: Convert branch_id to UUID if provided
        branch_uuid = None
        if branch_id and branch_id != '':
            try:
                branch_uuid = uuid.UUID(branch_id)
            except ValueError:
                current_app.logger.warning(f"Invalid branch_id format: {branch_id}")
...
@login_required
def add_supplier():
    """Add a new supplier using FormController."""
    if not has_permission(current_user, 'supplier', 'add'):
        flash('You do not have permission to add suppliers', 'danger')
        return redirect(url_for('supplier_views.supplier_list'))
    
    # Import controller locally to avoid circular imports
...
def edit_supplier(supplier_id):
    """Edit an existing supplier using FormController."""
...
def view_supplier(supplier_id):
    ...
    try:
        ...
        # Get supplier from database directly (to avoid transaction issues)
        with get_db_session(read_only=True) as session:
            ...
            # Convert to dictionary if needed for template compatibility
            supplier_dict = {
        ...
        # Get balance info (dummy data for now as service not available)
        balance_info = {
            'total_invoiced': 0,
            'total_paid': 0,
            'balance_due': 0
...
def supplier_invoice_list():
    ...
    if not has_permission(current_user, 'supplier_invoice', 'view'):
        flash('You do not have permission to view supplier invoices', 'danger')
        return redirect(url_for('supplier_views.supplier_list'))
    ...
    try:
        ...
        # Calculate summary metrics from the invoices data
        ...
        # Calculate total of all invoices displayed in the list
        listed_total = 0
        for invoice in invoices:
            # Try to get total_amount in different ways
        ...
        summary = {
            ...
            'listed_total': round(listed_total, 2)  # Add the listed total to summary
        }
            
        return render_template(
            'supplier/supplier_invoice_list.html',
            invoices=invoices,
            suppliers=suppliers,  # Add this line to pass suppliers to template
            form=form,
            page=page,
            per_page=per_page,
            total=total,
        ...
        # Include an empty summary for the error case
        return render_template(
            'supplier/supplier_invoice_list.html', 
            form=form, 
            invoices=[], 
            suppliers=[],  # Add empty suppliers list
...
def add_supplier_invoice():
    """Create a new supplier invoice using FormController."""
    if not has_permission(current_user, 'supplier_invoice', 'add'):
        flash('You do not have permission to create invoices', 'danger')
        return redirect(url_for('supplier_views.supplier_list'))
    
    # Check if we're coming from a PO
    po_id = request.args.get('po_id')
    
    # Import controller locally
    from app.controllers.supplier_controller import SupplierInvoiceFormController
    
    # Create controller with or without pre-populated PO data
    if po_id:
...
def view_supplier_invoice(invoice_id):
    ...
    try:
        ...
        # Get the associated purchase order directly
        po_data = None
        if invoice.get('po_id'):
            try:
                # Just set po_number from invoice data if available, don't fetch full PO
                if not invoice.get('po_number'):
                    # Try to get PO number without full PO details
        ...
        # Extract supplier from invoice data
        supplier = {
            ...
            'pan_number': '',  # Default empty string
            'tax_type': 'Regular',  # Default value
            'supplier_category': ''  # Default empty string
        }
        
        # Get payments from invoice data
        payments = invoice.get('payments', [])
        
        # Calculate subtotal and taxable value
        subtotal_sum = 0
        taxable_value_sum = 0
        
        if 'line_items' in invoice:
            for line in invoice['line_items']:
                # Calculate subtotal (pack_purchase_price * units)
                ...
                # Get taxable amount directly if available, otherwise calculate it
        ...
        # Add calculated values to the invoice data
        ...
        gst_summary = []
        if 'line_items' in invoice:
            # Group by HSN code and GST rate
            gstin_groups = {}
            for line in invoice['line_items']:
        ...
        return render_template(
            ...
            po_data=po_data  # Pass the PO data to the template
        )
    except Exception as e:
...
def purchase_order_list():
    ...
    try:
        ...
        # ENHANCEMENT: Check which POs already have invoices
        if purchase_orders:
            po_invoice_map = {}
                ...
                try:
                    ...
                    if po.get('po_id'):
                        invoice_result = search_supplier_invoices(
                            ...
                            po_id=po.get('po_id'),
                            page=1,
                            per_page=1  # We only need to know if any exist
                        )
            ...
            # Add has_invoices flag to each PO
            for po in purchase_orders:
                po['has_invoices'] = po_invoice_map.get(str(po.get('po_id')), False)

        # CRITICAL FIX: Ensure all required variables are passed to template
        return render_template(
            'supplier/purchase_order_list.html',
            purchase_orders=purchase_orders,
            form=form,
            page=page,
            per_page=per_page,  # FIX: This was missing
            total=total
        )
    except Exception as e:
        ...
        # FIX: Ensure error response also has all required variables
        return render_template(
            'supplier/purchase_order_list.html', 
            form=form, 
            purchase_orders=[], 
            page=1, 
            per_page=20,  # FIX: Add missing per_page
            total=0
        )
                          

# In app/views/supplier_views.py, around line 495
@supplier_views_bp.route('/purchase-order/add', methods=['GET', 'POST'])
@login_required
def add_purchase_order():
    """Add new purchase order with enhanced validation handling"""
    if not has_permission(current_user, 'purchase_order', 'create'):
    ...
    try:
        ...
        # If it's a string (rendered template), return it directly
        ...
        # For any other type, log and return to form
        ...
        # Parse multi-line validation errors for better display
        if '\n' in error_msg:
            error_lines = error_msg.split('\n')
            flash(error_lines[0], 'error')  # Main error message
            
            # Add individual validation errors as separate flash messages
...
def approve_purchase_order(po_id):
    ...
    if not has_permission(current_user, 'purchase_order', 'edit'):
...
def view_purchase_order(po_id):
    """View purchase order details - UPDATED to use stored GST values only"""
    if not has_permission(current_user, 'purchase_order', 'view'):
        flash('You do not have permission to view purchase orders', 'danger')
        return redirect(url_for('supplier_views.purchase_order_list'))
    ...
    try:
        ...
        # Create an instance of the cancel form
        cancel_form = PurchaseOrderCancelForm()
        
        # Log the PO ID for debugging
        current_app.logger.info(f"Attempting to view PO with ID: {po_id}")
        
        # Ensure po_id is a valid UUID string format before conversion
        try:
        ...
        # Get the PO data (now uses only stored values, no dynamic calculations)
        ...
        # Get the supplier data from the PO's supplier_id
        supplier = None
        ...
        # The get_purchase_order_by_id now returns stored GST calculations:
        # - calculated_subtotal (from stored taxable_amount)
        # - calculated_total_gst (from stored total_gst)  
        # - calculated_total_with_gst (aggregated total)
        # And each line item contains stored values:
        # - taxable_amount, total_gst, line_total, cgst, sgst, igst
        
        # Create a simple timeline array (keep existing functionality)
        timeline = []
        ...
        # Log the stored totals for debugging
...
def update_po_status(po_id):
    ...
    if not has_permission(current_user, 'purchase_order', 'edit'):
        flash('You do not have permission to update purchase orders', 'danger')
        return redirect(url_for('supplier_views.view_purchase_order', po_id=po_id))
...
def print_purchase_order(po_id):
    """Print purchase order - FIXED supplier variable error"""
    if not has_permission(current_user, 'purchase_order', 'view'):
        flash('You do not have permission to print purchase orders', 'danger')
        return redirect(url_for('supplier_views.purchase_order_list'))
    ...
    try:
        ...
        # FIX: Get supplier data from the PO's supplier_id
        supplier = None
        if po.get('supplier_id'):
            try:
                supplier = get_supplier_by_id(
        ...
        # Now supplier is properly defined
        return render_template(
            'supplier/print_purchase_order.html',
            po=po,
            purchase_order=po,  # Keep for backward compatibility
            supplier=supplier   # Now properly defined
        )
    except Exception as e:
...
def print_supplier_invoice(invoice_id):
    """Print supplier invoice - formatted like PO print"""
    if not has_permission(current_user, 'supplier_invoice', 'view'):
        flash('You do not have permission to print invoices', 'danger')
        return redirect(url_for('supplier_views.supplier_invoice_list'))
    ...
    try:
        ...
        # Get hospital info (you may need to adjust this based on your hospital model)
        hospital = None
        try:
            with get_db_session(read_only=True) as session:
...
def record_payment(invoice_id):
    ...
    if not has_permission(current_user, 'supplier_payment', 'add'):
        flash('You do not have permission to record payments', 'danger')
        return redirect(url_for('supplier_views.view_supplier_invoice', invoice_id=invoice_id))
...
def payment_history(supplier_id):
    ...
    if not has_permission(current_user, 'supplier_payment', 'view'):
        flash('You do not have permission to view payment history', 'danger')
        return redirect(url_for('supplier_views.supplier_list'))
...
def pending_invoices():
    ...
    if not has_permission(current_user, 'supplier_invoice', 'view'):
        flash('You do not have permission to view pending invoices', 'danger')
        return redirect(url_for('supplier_views.supplier_list'))
...
def get_medicine_api(medicine_id):
    ...
    try:
        with get_db_session(read_only=True) as session:
            ...
            # Check what attributes are available and use defaults if needed
            try:
                medicine_dict = {}
                ...
                # Check various price attributes and use the first available one
                ...
                # Override MRP if some other attribute exists
                ...
                # Return a simple structure with defaults in case of attribute errors
                return jsonify({
...
def cancel_supplier_invoice(invoice_id):
    ...
    if not has_permission(current_user, 'supplier_invoice', 'delete'):
...
def edit_supplier_invoice(invoice_id):
    ...
    if not has_permission(current_user, 'supplier_invoice', 'edit'):
...
def edit_purchase_order(po_id):
    """Edit purchase order using centralized framework"""
    if not has_permission(current_user, 'purchase_order', 'edit'):
        flash('You do not have permission to edit purchase orders', 'danger')
        return redirect(url_for('supplier_views.purchase_order_list'))
    
    try:
        # ADDED: Check PO status before allowing edit
        from app.services.supplier_service import get_purchase_order_by_id
...
def preview_po_gst():
    ...
    try:
        ...
        # Get hospital state for interstate calculation - FIXED with imports
        from app.services.database_service import get_db_session
        from app.models.master import Hospital  # ADD this line
        ...
        # Calculate preview using our standardized function
