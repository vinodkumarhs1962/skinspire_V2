...
def register():
    ...
    if form.validate_on_submit():
        try:
            ...
            # Store user input values for later use
            phone_number = form.phone.data
            user_type = form.user_type.data
            
            with get_db_session() as db_session:
                # Check if user already exists
                existing_user = db_session.query(User).filter_by(user_id=phone_number).first()
                ...
                current_app.logger.info(f"Created user account for {phone_number}")
            ...
            current_app.logger.info(f"User registered successfully: {phone_number}")
            
            # Check if verification is required
            from app.services.verification_service import VerificationService
            
            with get_db_session() as verification_session:
                user = verification_session.query(User).filter_by(user_id=phone_number).first()
                if user:
                    phone_required, email_required = VerificationService.verification_required(user)
                    
                    # Store important data in session
                    flask_session['new_user_id'] = phone_number
                    
                    # If staff, set staff registration user ID and redirect to approval form
                    if user_type == 'staff':
                        flask_session['staff_registration_user_id'] = phone_number
                        current_app.logger.info(f"Staff registration - redirecting to approval form for {phone_number}")
...
@login_required
@verification_required  # Add this decorator to enforce verification
def dashboard():
