...
def record_payment(invoice_id):
    ...
    try:
        ...
        current_app.logger.info(f"Form choices set up successfully. Suppliers: {len(suppliers) if 'suppliers' in locals() else 0}")
    ...
    try:
        ...
        # ADD DEBUGGING HERE
        current_app.logger.info(f"Invoice data keys: {list(invoice.keys())}")
        current_app.logger.info(f"Balance due: {invoice.get('balance_due')} (type: {type(invoice.get('balance_due'))})")
        current_app.logger.info(f"Total amount: {invoice.get('total_amount')} (type: {type(invoice.get('total_amount'))})")
        current_app.logger.info(f"Supplier ID: {invoice.get('supplier_id')} (type: {type(invoice.get('supplier_id'))})")
        ...
        if request.method == 'GET':
            ...
            # Use total amount if balance_due is 0 or None
            ...
            # Set supplier and invoice
            supplier_id = invoice.get('supplier_id')
            if supplier_id:
                form.supplier_id.data = str(supplier_id)
            ...
            # Set currency and exchange rate from invoice
            form.currency_code.data = invoice.get('currency_code', 'INR')
            form.exchange_rate.data = float(invoice.get('exchange_rate', 1.0))
            
            # Set branch from invoice if available
            if hasattr(form, 'branch_id') and invoice.get('branch_id'):
            ...
            # Set notes with invoice reference
            form.notes.data = f"Payment for invoice {invoice_number}"
            
            # ADD MORE DEBUGGING
            current_app.logger.info(f"Form populated - Amount: {form.amount.data}, Supplier: {form.supplier_id.data}")
...
def payment_list():
    ...
    try:
        ...
        return render_template('supplier/payment_list.html', 
                             payments=[], total=0, payment_config=PAYMENT_CONFIG)
...
def get_invoice_choices_for_supplier(supplier_id):
    """Get invoice choices for a specific supplier (AJAX endpoint)"""
    try:
        from app.utils.form_helpers import populate_invoice_choices_for_supplier
        from app.forms.supplier_forms import SupplierPaymentForm
        
        # Create a mock form to use the helper function
        form = SupplierPaymentForm()
        
        # Populate invoice choices
        populate_invoice_choices_for_supplier(form, current_user, uuid.UUID(supplier_id))
        
        # Return choices as JSON
        choices = [{'value': choice[0], 'text': choice[1]} for choice in form.invoice_id.choices]
