...
// Modified initializePatientSearch in invoice.js

function initializePatientSearch() {
    // Store original elements (preserved from original implementation)
    const patientSearch = document.getElementById('patient-search');
    ...
    // Early exit if elements aren't found (preserved from original implementation)
    ...
    try {
        // Try to use new PatientSearch component if available
        if (typeof window.PatientSearch === 'function') {
            console.log("Using enhanced patient search component");
            
            // Initialize new component
            const enhancedSearch = new window.PatientSearch({
                ...
                // Handle selection with behavior matching original implementation
                onSelect: function(patient) {
                    console.log("Patient selected:", patient.name, "ID:", patient.id);
                    
                    // Update the patient info display (same as original)
                    if (patientNameDisplay) patientNameDisplay.textContent = patient.name;
                    if (patientMRNDisplay) patientMRNDisplay.textContent = `MRN: ${patient.mrn}`;
                    if (patientContactDisplay) patientContactDisplay.textContent = patient.contact || '';
                    if (patientInfo) patientInfo.classList.remove('hidden');
                    
                    // Additional behavior from original implementation
                    if (invoiceForm) {
                        invoiceForm.setAttribute('data-patient-id', patient.id);
                    ...
                    patientSearch.dispatchEvent(new Event('change', { bubbles: true }));
                },
                
                // Handle errors
                onError: function(error) {
                    console.error("Enhanced patient search error:", error);
                    console.warn("Falling back to legacy search implementation");
                    
                    // If error occurs, initialize legacy behavior
                    initializeLegacyPatientSearch();
                }
            });
            
            // Return early since we're using the enhanced component
            return;
        } else {
            console.log("Enhanced patient search not available, using legacy implementation");
            initializeLegacyPatientSearch();
        }
    } catch (e) {
        console.error("Error initializing enhanced patient search:", e);
        console.warn("Falling back to legacy search implementation");
        
        // Fall back to legacy search implementation
        initializeLegacyPatientSearch();
    }
    
    // Original implementation wrapped as a separate function for fallback
    function initializeLegacyPatientSearch() {
        // Ensure patient_id field exists (from original implementation)
        ...
        // Debounce function (from original implementation)
        const debounce = (func, delay) => {
            let timeout;
            return function(...args) {
                clearTimeout(timeout);
        ...
        // Search patients (from original implementation)
        ...
        // Add event listener for patient search (from original implementation)
        patientSearch.addEventListener('input', function() {
            const query = this.value.trim();
            searchPatients(query);
        });
        
        // Click on search field shows all patients (like in original implementation)
        ...
        // Handle direct entry of patient name (from original implementation)
        ...
        // Hide results when clicking elsewhere (from original implementation)
        document.addEventListener('click', function(e) {
