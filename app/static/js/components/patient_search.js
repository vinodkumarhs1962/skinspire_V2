...
class PatientSearch {
    ...
    init() {
        ...
        // Disable browser autocomplete to prevent browser suggestions
        if (this.input) {
    ...
    _addEventListeners() {
        ...
        // Replace focus event with click event for first interaction
        this.input.addEventListener('click', (e) => {
            // Only trigger on first click if field is empty
            if (!this.input.dataset.clicked && !this.input.value.trim()) {
                this.input.dataset.clicked = 'true';
                this.searchPatients('');
            }
        });
        
        // Reset click flag when input is cleared
        this.input.addEventListener('input', (e) => {
            if (!e.target.value.trim()) {
                this.input.dataset.clicked = 'false';
            }
        });
    ...
    searchPatients(query) {
        // Store a reference to the query for later use
        ...
        // Add width restriction to the results container
        this.results.style.maxWidth = '400px';
        this.results.style.width = 'auto';

        // Build query URL properly avoiding duplicate parameters
        let searchUrl = this.options.searchEndpoint;
        
        // Detect if endpoint already has query parameters
        if (searchUrl.includes('?')) {
            // URL already has parameters
            if (searchUrl.includes('q=')) {
                // Replace existing q parameter to avoid duplication
                searchUrl = searchUrl.replace(/q=[^&]*/, `q=${encodeURIComponent(query)}`);
            } else {
                // Add q parameter with &
                searchUrl += `&q=${encodeURIComponent(query)}`;
            }
            
            // Check if limit parameter already exists
            if (!searchUrl.includes('limit=')) {
                searchUrl += `&limit=${this.options.limit}`;
            }
        } else {
            // No parameters yet, add first parameter with ?
            searchUrl += `?q=${encodeURIComponent(query)}&limit=${this.options.limit}`;
        }
        ...
        .then(data => {
            ...
            // Store the selected patient ID and name as class properties
            // This makes them accessible outside for form validation
            if (Array.isArray(data) && data.length > 0 && this.patientIdInput) {
                this.selectedPatientId = this.patientIdInput.value;
                this.selectedPatientName = this.input.value;
            }
        })
        .catch(error => {
