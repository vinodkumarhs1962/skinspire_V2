...
class SupplierFormController(FormController):
    ...
    def process_form(self, form, *args, **kwargs):
        """Process the form data - aligned with original field names"""
        try:
...
class SupplierInvoiceFormController(FormController):
    ...
    def get_additional_context(self, *args, **kwargs):
        ...
        try:
            ...
            # Get suppliers for dropdown (using search_suppliers)
            ...
            # Get hospital state code and other context data
            with get_db_session(read_only=True) as session:
                ...
                context['branches'] = [get_detached_copy(branch) for branch in branches]
                
                # Get purchase orders - create detached copies
                ...
                # Create line item form with medicine choices
            ...
            current_app.logger.error(f"Error getting context: {str(e)}", exc_info=True)
        
        # Add flag to indicate if we have pre-populated PO data
    ...
    def process_form(self, form, line_items=None, *args, **kwargs):
        """
        Process the invoice form data
        UPDATED: Enhanced free item validation and interstate handling
        """
        try:
            # Import locally to avoid circular import
            ...
            branch_id = form.branch_id.data if hasattr(form, 'branch_id') else None
...
class SupplierInvoiceEditController(FormController):
    ...
    def process_form(self, form, *args, **kwargs):
        """Process form submission - FIXED to handle choices properly"""
        try:
    ...
    def handle_request(self, *args, **kwargs):
        """Override handle_request to ensure choices are always set"""
        from flask import request
        
        try:
            if request.method == 'POST':
                # For POST requests, we bypass WTForms validation and use custom processing
                form = self.get_form(*args, **kwargs)
                try:
...
class PurchaseOrderFormController(FormController):
    ...
    def get_additional_context(self, *args, **kwargs):
        """Get suppliers, medicines, and purchase orders for dropdowns"""
        context = {}
        try:
            from app.services.database_service import get_db_session
    ...
    def process_form(self, form, *args, **kwargs):
        """Process the form data for creation only"""
        try:
            # Import the validation-enabled service function
...
class PurchaseOrderEditController(FormController):
    ...
    def process_form(self, form, *args, **kwargs):
        """Process form submission - BYPASS form validation, use custom validation"""
        try:
            ...
            for idx, item in enumerate(line_items_data):
                current_app.logger.info(f"Line {idx + 1}: Qty={item['units']}, Price={item['pack_purchase_price']}, Free={item['is_free_item']}")
...
class SupplierFormController(FormController):
    ...
    def get_additional_context(self, *args, **kwargs):
        """Get additional context including branch selection"""
        ...
        try:
            ...
            # NEW: Get branches for selection
            with get_db_session(read_only=True) as session:
                branches = session.query(Branch).filter_by(
                    hospital_id=current_user.hospital_id,
                    is_active=True
                ...
                # NEW: Get user's default branch
                from app.services.supplier_service import get_user_branch_id
                default_branch_id = get_user_branch_id(current_user.user_id, current_user.hospital_id)
                context['default_branch_id'] = default_branch_id
                
        except Exception as e:
            current_app.logger.error(f"Error getting branches: {str(e)}")
            context['branches'] = []
...
class PurchaseOrderFormController(FormController):
    ...
    def get_additional_context(self, *args, **kwargs):
        """Get suppliers, medicines, and purchase orders for dropdowns - FIXED session management"""
        context = {}
        try:
            from app.services.database_service import get_db_session, get_detached_copy
            from app.models.master import Supplier, Medicine, Branch
            ...
            # CRITICAL FIX: Use single session for all operations and create detached copies
            with get_db_session(read_only=True) as session:
                current_app.logger.info("Getting additional context for PO form - starting database operations")
                
                # NEW: Get user branch context safely
                user_branch_id = None
                try:
                    from app.services.supplier_service import get_user_branch_id
                    user_branch_id = get_user_branch_id(current_user.user_id, current_user.hospital_id)
                    current_app.logger.info(f"User branch ID: {user_branch_id}")
                except Exception as branch_error:
                    current_app.logger.warning(f"Could not get user branch: {str(branch_error)}")
                
                # Get suppliers (filtered by branch if user has specific branch)
                supplier_query = session.query(Supplier).filter_by(
                    hospital_id=current_user.hospital_id,
                    status='active'
                )
                ...
                if user_branch_id:
                    supplier_query = supplier_query.filter_by(branch_id=user_branch_id)
                
                suppliers = supplier_query.all()
                current_app.logger.info(f"Found {len(suppliers)} suppliers")
                
                # CRITICAL FIX: Create detached copies while session is active
                context['suppliers'] = [get_detached_copy(supplier) for supplier in suppliers]
                
                # Get medicines
                medicines = session.query(Medicine).filter_by(
                    hospital_id=current_user.hospital_id
                ).order_by(Medicine.medicine_name).limit(1000).all()  # Limit for performance
                current_app.logger.info(f"Found {len(medicines)} medicines")
                ...
                current_app.logger.info(f"Found {len(branches)} branches")
                
                # CRITICAL FIX: Create detached copies while session is active
                context['branches'] = [get_detached_copy(branch) for branch in branches]
                context['user_branch_id'] = user_branch_id
                
                current_app.logger.info("Successfully created detached copies of all data")
                
                # FIXED: Create line item form with medicine choices OUTSIDE of nested session
                from app.forms.supplier_forms import PurchaseOrderLineForm
                line_item_form = PurchaseOrderLineForm()
                
                # Set medicine choices using detached data
                medicine_choices = [('', 'Select Medicine')] + [
                    (str(med.medicine_id), med.medicine_name) for med in context['medicines']
                ]
                line_item_form.medicine_id.choices = medicine_choices
                context['line_item_form'] = line_item_form
                
                current_app.logger.info("Additional context preparation completed successfully")

        except Exception as e:
            current_app.logger.error(f"Error getting context: {str(e)}", exc_info=True)
            # FALLBACK: Provide minimal context to prevent complete failure
            context = {
                'suppliers': [],
                'medicines': [],
                'branches': [],
    ...
    def get_form(self, *args, **kwargs):
        """Get form instance for creation only - FIXED using dictionary approach"""
        form = super().get_form(*args, **kwargs)
        
        try:
            # Set supplier choices using context data (no additional DB calls)
            context = self.get_additional_context()
            if 'suppliers' in context and context['suppliers']:
                form.supplier_id.choices = [('', 'Select Supplier')] + [
                    (str(s['supplier_id']), s['supplier_name']) for s in context['suppliers']
                ]
            else:
                form.supplier_id.choices = [('', 'Select Supplier')]
            ...
            current_app.logger.info("Form initialization completed successfully")
            
        except Exception as e:
            current_app.logger.error(f"Error initializing form: {str(e)}")
            # Fallback to minimal form setup
            form.supplier_id.choices = [('', 'Select Supplier')]
            form.po_number.data = "PO/NEW/00001"
        
        return form
    
    def generate_po_number(self):
        """Generate a new PO number - FIXED session management"""
        from datetime import datetime
        from flask_login import current_user
        from app.services.database_service import get_db_session
        from app.models.transaction import PurchaseOrderHeader
        ...
        try:
            ...
            # FIXED: Use completely separate session for PO number generation
            with get_db_session(read_only=True) as po_session:
                # Get the last PO number for this hospital and financial year
                last_po = po_session.query(PurchaseOrderHeader).filter(
                    PurchaseOrderHeader.hospital_id == current_user.hospital_id,
                    PurchaseOrderHeader.po_number.like(f"PO/{fin_year}/%")
                ).order_by(PurchaseOrderHeader.po_number.desc()).first()
            ...
            current_app.logger.info(f"Generated PO number: {po_number}")
            return po_number
            
        except Exception as e:
            current_app.logger.error(f"Error generating PO number: {str(e)}")
            # Fallback PO number
            return f"PO/NEW/{datetime.now().strftime('%Y%m%d%H%M%S')}"
    
    def get_user_branch_id(self, user_id, hospital_id):
        """Get user's branch ID with simple direct query"""
        try:
            from app.services.database_service import get_db_session
            ...
            with get_db_session() as session:
                ...
                if user and user.entity_type == 'staff' and user.entity_id:
                    ...
                    if staff and staff.branch_id:
                        current_app.logger.info(f"Found user branch: {staff.branch_id}")
                        return staff.branch_id
                
                current_app.logger.info("No specific branch found for user")
                return None
                
        except Exception as e:
            current_app.logger.error(f"Error getting user branch: {str(e)}")
            return None

    def process_form(self, form, *args, **kwargs):
        """Process the form data for creation only - SIMPLE branch handling"""
        try:
            from app.services.supplier_service import create_purchase_order_with_validation
            from flask_login import current_user
            import uuid
            ...
            # SIMPLE: Get user's branch ID directly
            branch_id = self.get_user_branch_id(current_user.user_id, current_user.hospital_id)
            
            # Create PO data with branch_id
            po_data = {
                'po_number': form.po_number.data,
                'supplier_id': uuid.UUID(form.supplier_id.data),
                'po_date': form.po_date.data,
                ...
                'status': 'approved' if is_approved else 'draft',
                'line_items': line_items,
                'branch_id': branch_id  # Add branch_id directly to PO data
            }
            
            current_app.logger.info(f"Creating PO with {len(line_items)} line items for branch {branch_id}")

            # Use validation-enabled service function
            result = create_purchase_order_with_validation(
                hospital_id=current_user.hospital_id,
                po_data=po_data,  # branch_id is included in po_data
                current_user_id=current_user.user_id
            )
