# app/utils/menu_utils.py - BACKWARD COMPATIBLE FIX
# MINIMAL CHANGES: Add safe URL generation without changing existing structure
...
def safe_url_for(endpoint, **values):
    """
    ADDED: Helper function to safely create URLs for potentially missing blueprints
...
def generate_menu_for_role(role):
    ...
    if role == 'staff':
        ...
        # 2. Inventory Management - FIXED with safe_url_for
        menu.append({
            'name': 'Inventory',
            'url': '#',
            'icon': 'cube',
            ...
            'children': [
                {
                    'name': 'Stock List',
                    'url': safe_url_for('inventory_views.inventory_list'),  # FIXED: Use safe_url_for
                    'icon': 'archive'
                },
                {
                    'name': 'Stock Movement',
                    'url': safe_url_for('inventory_views.inventory_movement'),  # FIXED: Use safe_url_for
                    'icon': 'arrow-right'
                },
                {
                    'name': 'Stock Adjustment',
                    'url': safe_url_for('inventory_views.stock_adjustment'),  # FIXED: Use safe_url_for
                    'icon': 'pencil'
                },
                {
                    'name': 'Low Stock',
                    'url': safe_url_for('inventory_views.low_stock'),  # FIXED: Use safe_url_for
                    'icon': 'exclamation'
                },
                {
                    'name': 'Expiring Stock',
                    'url': safe_url_for('inventory_views.expiring_stock'),  # FIXED: Use safe_url_for
                    'icon': 'clock'
                },
                {
                    'name': 'Batch Management',
                    'url': safe_url_for('inventory_views.batch_management'),  # FIXED: Use safe_url_for
                    'icon': 'tag'
                }
            ]
        })
        
        # 3. Supplier Management - KEPT as-is (uses direct URLs)
        menu.append({
            'name': 'Suppliers',
        ...
        # 4. Financial Reports - FIXED with safe_url_for
        menu.append({
            ...
            'children': [
                {
                    'name': 'Balance Sheet',
                    'url': safe_url_for('gl_views.generate_report', report_type='balance_sheet'),  # FIXED: Use safe_url_for
                    'icon': 'document'
                },
                {
                    'name': 'Profit & Loss',
                    'url': safe_url_for('gl_views.generate_report', report_type='profit_loss'),  # FIXED: Use safe_url_for
                    'icon': 'chart-bar'
                },
                {
                    'name': 'Trial Balance',
                    'url': safe_url_for('gl_views.generate_report', report_type='trial_balance'),  # FIXED: Use safe_url_for
                    'icon': 'calculator'
                },
                {
                    'name': 'GST Reports',
                    'url': safe_url_for('gl_views.gst_reports'),  # FIXED: Use safe_url_for
                    'icon': 'document-report'
                },
                {
                    'name': 'Account Reconciliation',
                    'url': safe_url_for('gl_views.account_reconciliation'),  # FIXED: Use safe_url_for
                    'icon': 'check-circle'
                }
            ]
        })
        
        # 5. Patient Management - KEPT as-is (uses placeholder URLs)
        menu.append({
            'name': 'Patients',
            'url': '#',
            'icon': 'users',
        ...
        # 6. Appointments - KEPT as-is (uses placeholder URLs)
        menu.append({
            'name': 'Appointments',
            'url': '#',
            'icon': 'calendar',
        ...
        # 7. Administration - FIXED with safe_url_for
        menu.append({
            'name': 'Administration',
            'url': '#',
            'icon': 'cog',
            ...
            'children': [
                {
                    ...
                    'url': safe_url_for('admin_views.hospital_settings'),  # FIXED: Use safe_url_for
                    'icon': 'cog'
                },
                {
                    'name': 'System Admin Dashboard',
                    'url': safe_url_for('admin_views.system_admin_dashboard'),  # FIXED: Use safe_url_for
                    'icon': 'server'
                },
                {
                    'name': 'Hospital Admin Dashboard',
                    'url': safe_url_for('admin_views.hospital_admin_dashboard'),  # FIXED: Use safe_url_for
                    'icon': 'building'
                },
                {
        ...
        # Always have settings as the last item - KEPT as-is
        menu.append({
            'name': 'Settings',
        ...
        # Patient-specific menu items - KEPT as-is
        menu.append({
...
# KEPT: Existing get_menu_items function unchanged for backward compatibility
