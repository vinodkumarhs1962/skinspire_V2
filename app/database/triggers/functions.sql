...
-- Password hashing function - DISABLED in favor of Werkzeug hashing in Python code
...
BEGIN
    ...
    -- Simply return the record unchanged. The Python code will handle the hashing.
    RAISE NOTICE 'Password hash trigger called but disabled - using Werkzeug hashing in Python instead';
    RETURN NEW;
END;
$$ LANGUAGE plpgsql;
...
BEGIN
    ...
    IF EXISTS (
        ...
        -- Password hashing trigger is now DISABLED
        -- Password hashing is now handled by Werkzeug in Python code
        -- DROP TRIGGER IF EXISTS hash_password ON users;
        -- CREATE TRIGGER hash_password
        -- BEFORE INSERT OR UPDATE ON users
        -- FOR EACH ROW
        -- EXECUTE FUNCTION hash_password();
        RAISE NOTICE 'Password hashing trigger has been disabled - now using Werkzeug in Python code';
