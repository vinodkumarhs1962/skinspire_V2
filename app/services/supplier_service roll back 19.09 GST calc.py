...
def get_medicines_with_last_mrp(
    session: Session,
    hospital_id: uuid.UUID,
    branch_id: Optional[uuid.UUID] = None
) -> List[Dict]:
    """
    Get all active medicines with their MRP from medicine master.
    Used for populating the medicine dropdown in PO creation.
    
    Args:
        session: Database session
        hospital_id: Hospital UUID
        branch_id: Optional branch UUID for filtering
        
    Returns:
        List of medicine dictionaries with MRP data
    """
    try:
        from app.models.master import Medicine
        
        # Get all active medicines
        medicines_query = session.query(Medicine).filter(
        ...
        if branch_id:
            medicines_query = medicines_query.filter(Medicine.branch_id == branch_id)
        
        medicines = medicines_query.order_by(Medicine.medicine_name).all()
        ...
        for medicine in medicines:
            medicine_dict = {
                ...
                'manufacturer_name': medicine.manufacturer.manufacturer_name if medicine.manufacturer else None,
                'category_name': medicine.category.category_name if medicine.category else None,
                'hsn_code': medicine.hsn_code or '',
                'gst_rate': float(medicine.gst_rate or 12),
                'pack_size': int(medicine.pack_size or 1),
                'unit_type': medicine.unit_type or 'Strip',
                'unit_of_measure': medicine.unit_of_measure or 'Tablets',
                
                # MRP fields from medicine master
                'mrp': float(medicine.mrp or 0),
                'last_mrp': float(medicine.mrp or 0),  # For compatibility
                ...
                'current_stock': int(medicine.current_stock or 0)
            }
            medicines_with_mrp.append(medicine_dict)
        
        logger.info(f"Retrieved {len(medicines_with_mrp)} medicines with MRP data")
        return medicines_with_mrp
        
    except Exception as e:
        logger.error(f"Error fetching medicines with MRP: {str(e)}")
        raise


def get_supplier_last_purchase_price(
    session: Session,
    hospital_id: uuid.UUID,
    supplier_id: uuid.UUID,
    ...
    """
    Get the last purchase price and discount for a specific medicine from a supplier.
    ...
    Returns:
        Dictionary with last price info or None
    """
    try:
        from app.models.transaction import SupplierInvoice, SupplierInvoiceLine, PurchaseOrderHeader, PurchaseOrderLine
        from datetime import datetime
        
        # Check supplier invoices first
        last_invoice_purchase = session.query(
            SupplierInvoiceLine.pack_purchase_price,
            SupplierInvoiceLine.pack_mrp,
            SupplierInvoiceLine.discount_percent,
            ...
            SupplierInvoice.hospital_id == hospital_id,
            SupplierInvoice.supplier_id == supplier_id,
            SupplierInvoiceLine.medicine_id == medicine_id,
        ...
        if last_invoice_purchase:
            days_ago = (datetime.now().date() - last_invoice_purchase.invoice_date.date()).days
            
            return {
                'rate': float(last_invoice_purchase.pack_purchase_price),
                'mrp': float(last_invoice_purchase.pack_mrp) if last_invoice_purchase.pack_mrp else 0,
                'discount': float(last_invoice_purchase.discount_percent or 0),
                'days_ago': days_ago,
                'date': last_invoice_purchase.invoice_date.strftime('%Y-%m-%d'),
                'reference': f"Invoice: {last_invoice_purchase.supplier_invoice_number}",
                'source': 'invoice'
            }
        
        # Fallback to purchase orders
        last_po_purchase = session.query(
            PurchaseOrderLine.pack_purchase_price,
            PurchaseOrderLine.pack_mrp,
            PurchaseOrderLine.discount_percent,
            PurchaseOrderHeader.po_date,
            PurchaseOrderHeader.po_number
        ).join(
            PurchaseOrderHeader,
            PurchaseOrderLine.po_id == PurchaseOrderHeader.po_id
        ).filter(
            PurchaseOrderHeader.hospital_id == hospital_id,
            PurchaseOrderHeader.supplier_id == supplier_id,
            PurchaseOrderLine.medicine_id == medicine_id,
            PurchaseOrderHeader.status.notin_(['Cancelled', 'Draft'])
        ).order_by(
            PurchaseOrderHeader.po_date.desc()
        ).first()
        
        if last_po_purchase:
            days_ago = (datetime.now() - last_po_purchase.po_date).days
            
            return {
                'rate': float(last_po_purchase.pack_purchase_price),
                'mrp': float(last_po_purchase.pack_mrp) if last_po_purchase.pack_mrp else 0,
                'discount': float(last_po_purchase.discount_percent or 0),
                'days_ago': days_ago,
                'date': last_po_purchase.po_date.strftime('%Y-%m-%d'),
                'reference': f"PO: {last_po_purchase.po_number}",
        ...
        logger.warning(f"Error fetching last purchase price: {str(e)}")
        return None


def get_supplier_average_metrics(
    session: Session,
    hospital_id: uuid.UUID,
    supplier_id: uuid.UUID,
    ...
    """
    Calculate average metrics for a supplier.
    
    Args:
        session: Database session
        hospital_id: Hospital UUID
        supplier_id: Supplier UUID
        medicine_id: Optional medicine UUID
        months: Number of months to analyze
        
    Returns:
        Dictionary with supplier metrics
    """
    try:
        ...
        avg_discount_query = session.query(
            func.avg(SupplierInvoiceLine.discount_percent)
        ).join(
            SupplierInvoice,
            SupplierInvoiceLine.invoice_id == SupplierInvoice.invoice_id
        ).filter(
            ...
            SupplierInvoiceLine.discount_percent > 0
        )
        
        if medicine_id:
            avg_discount_query = avg_discount_query.filter(
                SupplierInvoiceLine.medicine_id == medicine_id
            )
        
        avg_discount = avg_discount_query.scalar() or 0
        
        return {
            'average_discount': round(float(avg_discount), 2),
            'months_analyzed': months,
            'supplier_id': str(supplier_id),
            'medicine_id': str(medicine_id) if medicine_id else None
        }
        
    except Exception as e:
        logger.warning(f"Error calculating supplier metrics: {str(e)}")
        return {
            'average_discount': 0,
            'months_analyzed': months,
            'error': str(e)
...
def update_medicine_prices_from_invoice(
    session: Session,
    hospital_id: uuid.UUID,
    invoice_lines: List,
    ...
    """
    Update medicine MRP and prices when processing supplier invoice.
    ...
    Args:
        ...
        invoice_number: Invoice number for audit
    ...
    try:
        ...
        for line in invoice_lines:
            ...
            if line.pack_mrp and line.pack_mrp > 0:
                if not medicine.mrp or float(line.pack_mrp) != float(medicine.mrp):
                    result = MedicineService.update_medicine_mrp(
                        session=session,
                        medicine_id=medicine.medicine_id,
                        new_mrp=line.pack_mrp,
                        effective_date=invoice_date.date() if hasattr(invoice_date, 'date') else invoice_date,
                        source='INVOICE',
                        reference_id=line.invoice_id
                    )
                    
                    if result['success']:
                        updates_summary['mrp_updated'] += 1
            
            # Update last purchase price
            if line.pack_purchase_price and line.pack_purchase_price > 0:
                effective_price = float(line.pack_purchase_price)
                if line.discount_percent:
                    effective_price = effective_price * (1 - float(line.discount_percent) / 100)
                
                if MedicineService.update_purchase_price(
                    session=session,
                    medicine_id=medicine.medicine_id,
                    new_price=effective_price
                ):
                    updates_summary['purchase_price_updated'] += 1
                
                # Validate margin
                if medicine.mrp:
                    margin_percent = ((float(medicine.mrp) - effective_price) / float(medicine.mrp)) * 100
                    if margin_percent < 10:
                        warning = f"Low margin for {medicine.medicine_name}: {margin_percent:.1f}%"
                        updates_summary['warnings'].append(warning)
        
        session.flush()
        
        logger.info(f"Price update for Invoice {invoice_number}: "
                   f"MRP: {updates_summary['mrp_updated']}, "
                   f"Purchase: {updates_summary['purchase_price_updated']}")
        
        return updates_summary
        
    except Exception as e:
        logger.error(f"Error updating medicine prices: {str(e)}")
        raise