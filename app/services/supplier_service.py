...
def _record_supplier_payment(
    ...
    """
    Internal payment recording function - ENHANCED with detailed logging
    """
    try:
        logger.info("🔍 PAYMENT DEBUG: Starting payment validation")
        logger.info(f"🔍 PAYMENT DEBUG: create_gl_entries parameter = {create_gl_entries}")
        
        # Pass the session to validation to avoid creating new sessions
        ...
        logger.info("🔍 PAYMENT DEBUG: Validating supplier")
        ...
        logger.info(f"🔍 PAYMENT DEBUG: Found supplier: {supplier_name}")
        ...
        if payment_data.get('invoice_id'):
            ...
            logger.info(f"🔍 PAYMENT DEBUG: Invoice {invoice_data['supplier_invoice_number']}: Total={invoice_data['total_amount']}, Paid={existing_payment_total}")
        
        logger.info("🔍 PAYMENT DEBUG: Creating payment record")
        ...
        if float(payment.amount) <= auto_approve_limit:
            ...
            logger.info(f"🔍 PAYMENT DEBUG: Payment auto-approved (amount: {payment.amount} <= limit: {auto_approve_limit})")
        
        session.add(payment)
        session.flush()
        
        logger.info(f"🔍 PAYMENT DEBUG: Payment record created with ID: {payment.payment_id}")
        logger.info(f"🔍 PAYMENT DEBUG: Payment status: {payment.workflow_status}")
        ...
        if invoice and invoice_data:
            ...
            if total_payments >= invoice_total:
                ...
                logger.info(f"🔍 PAYMENT DEBUG: Invoice fully paid - status set to 'paid'")
            elif total_payments > 0:
                invoice.payment_status = 'partial'
                remaining_balance = invoice_total - total_payments
                logger.info(f"🔍 PAYMENT DEBUG: Partial payment - status set to 'partial', balance: ₹{remaining_balance}")
            else:
                invoice.payment_status = 'unpaid'
                logger.info(f"🔍 PAYMENT DEBUG: No approved payments - status remains 'unpaid'")
            
            invoice.updated_at = datetime.now(timezone.utc)
            invoice.updated_by = current_user_id
            
            logger.info(f"🔍 PAYMENT DEBUG: Updated invoice payment status to: {invoice.payment_status}")
        
        session.flush()
        
        # ========================================================================
        # ENHANCED GL CREATION LOGIC - FIXED (NO DUPLICATES)
        # ========================================================================
        
        logger.info(f"🔍 PAYMENT DEBUG: GL Creation Check:")
        logger.info(f"🔍 PAYMENT DEBUG:   create_gl_entries = {create_gl_entries}")
        logger.info(f"🔍 PAYMENT DEBUG:   payment.workflow_status = {payment.workflow_status}")
        logger.info(f"🔍 PAYMENT DEBUG:   condition result = {payment.workflow_status == 'approved'}")
        
        # Create GL entries if payment is approved
        if payment.workflow_status == 'approved':
            logger.info("🔄 PAYMENT GL: Payment is approved - attempting GL creation")
            
            try:
                # STEP 1: Try standard GL entry creation (if requested)
                if create_gl_entries:
                    logger.info("🔄 PAYMENT GL: Attempting standard GL entry creation")
                    try:
                        ...
                        logger.info("✅ PAYMENT GL: Standard GL entries created successfully")
                    except Exception as std_gl_error:
                        logger.error(f"❌ PAYMENT GL: Standard GL creation failed: {str(std_gl_error)}")
                else:
                    logger.info("⚠️ PAYMENT GL: Standard GL creation skipped (create_gl_entries=False)")
                
                # STEP 2: ALWAYS try enhanced posting (SINGLE CALL)
                logger.info("🔄 PAYMENT GL: Attempting enhanced posting")
                
                from app.services.enhanced_posting_helper import EnhancedPostingHelper
                enhanced_helper = EnhancedPostingHelper()
                
                # Check if enhanced posting is enabled
                logger.info(f"🔍 PAYMENT GL: Enhanced posting enabled: {enhanced_helper.enabled}")
                
                if enhanced_helper.enabled:
                    ...
                    logger.info(f"🔄 PAYMENT GL: Enhanced posting result: {enhanced_result}")
                    
                    if enhanced_result['status'] == 'success':
                        logger.info(f"✅ PAYMENT GL: Enhanced posting completed: {enhanced_result.get('posting_reference', 'No reference')}")
                    elif enhanced_result['status'] == 'error':
                        logger.error(f"❌ PAYMENT GL: Enhanced posting failed: {enhanced_result.get('error', 'Unknown error')}")
                    elif enhanced_result['status'] == 'disabled':
                        logger.info(f"ℹ️ PAYMENT GL: Enhanced posting disabled: {enhanced_result.get('message', 'Not enabled')}")
                    else:
                        logger.warning(f"⚠️ PAYMENT GL: Enhanced posting unexpected status: {enhanced_result}")
                else:
                    logger.info("ℹ️ PAYMENT GL: Enhanced posting helper is disabled")
                    
            except Exception as gl_error:
                logger.error(f"❌ PAYMENT GL: Critical error in GL creation: {str(gl_error)}")
                logger.error(f"❌ PAYMENT GL: GL Error traceback:", exc_info=True)
                logger.warning("⚠️ PAYMENT GL: Payment will be recorded without GL entries")
                
        else:
            logger.info(f"ℹ️ PAYMENT GL: Payment not approved yet (status: {payment.workflow_status}) - GL creation skipped")
        ...
        logger.info(f"🔍 PAYMENT DEBUG: Payment processing completed successfully - ready for commit")
        return result
        
    except Exception as e:
        logger.error(f"❌ PAYMENT DEBUG: Error in _record_supplier_payment: {str(e)}")
        logger.error("❌ PAYMENT DEBUG: Full traceback:", exc_info=True)
        raise
