...
# app/services/supplier_service.py - COMPLETE PART 2: Supplier Management Functions
# REPLACE all supplier management functions with these
...
def search_supplier_invoices(
    ...
    """
    Search supplier invoices with filtering and pagination - UPDATED to use branch_service
    """
...
def _search_supplier_invoices(
    ...
    branch_id: Optional[uuid.UUID] = None,  # NEW: Added branch filtering
    page: int = 1,
    per_page: int = 20
) -> Dict:
    """
    Search supplier invoices using stored values only with branch support - UPDATED
    """
    try:
        logger.info(f"Searching supplier invoices for hospital {hospital_id}")
        ...
        if payment_status:
            logger.debug(f"Filtering by payment status: {payment_status}")
        ...
        # Convert to dictionaries using stored values only
        invoice_list = []
...
def record_supplier_payment(
    ...
    """
    Record a payment to a supplier - EXISTING FUNCTION (unchanged - already has branch support)
    """
...
def validate_payment_data(payment_data: Dict) -> List[str]:
    """
    Validate payment data before processing - EXISTING FUNCTION (unchanged)
    """
    errors = []
    
    try:
        # Required fields validation
        required_fields = ['supplier_id', 'amount', 'payment_date', 'payment_method']
        ...
        valid_methods = ['cash', 'cheque', 'bank_transfer', 'upi', 'card', 'others']
        ...
        # Invoice-specific validation if invoice_id is provided
        invoice_id = payment_data.get('invoice_id')
        if invoice_id:
            # Additional validation would be performed in the service layer
            pass
        
    except Exception as e:
        errors.append(f"Error validating payment data: {str(e)}")
...
def _validate_payment_data_internal(
    ...
    """
    Internal payment data validation with database checks - EXISTING FUNCTION (unchanged)
    """
    errors = []
...
def validate_payment_business_rules(
    ...
    """
    Validate business rules for supplier payments - EXISTING FUNCTION (unchanged)
    """
    errors = []
            ...
            if invoice:
                ...
                if total_with_new_payment > invoice_total:
                    ...
                    errors.append(
                        ...
                        f"Already paid:  Rs.{existing_payments:.2f}, "
                        f"Overpayment:  Rs.{overpayment:.2f}"
                    )
    
    except Exception as e:
        errors.append(f"Error validating business rules: {str(e)}")
...
# ===================================================================
# END OF COMPLETE SUPPLIER SERVICE
# Note: All existing internal functions like _record_supplier_payment, 
# _update_supplier_invoice, _get_supplier_invoice_by_id, _cancel_supplier_invoice
# remain unchanged as they already have proper functionality
# ===================================================================