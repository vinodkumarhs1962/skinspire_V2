...
def create_supplier(
    ...
    """
    # NEW: Determine branch_id if not provided
...
def search_suppliers(
    ...
    # NEW: Auto-determine branch if not specified and user provided
    if not branch_id and current_user_id:
        branch_id = get_user_branch_id(current_user_id, hospital_id)
    
    # Pass branch_id to internal function
    if session is not None:
        return _search_suppliers(
            session, hospital_id, name, category, gst_number, 
            status, blacklisted, branch_id, page, per_page
        )
...
def create_purchase_order(
    ...
    """
    # NEW: Determine branch if not provided
    if not branch_id:
        branch_id = get_user_branch_id(current_user_id, hospital_id)
        if not branch_id:
