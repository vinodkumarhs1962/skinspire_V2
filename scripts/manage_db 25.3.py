...
def restore_backup(file):
    ...
    try:
        # Check for explicit environment override from environment variable
        if 'FLASK_ENV' in os.environ:
            flask_env = os.environ['FLASK_ENV']
            # Map from Flask env to short env name
            if flask_env == 'testing':
                ...
                # Try to determine from DatabaseConfig
                try:
                    from app.config.db_config import DatabaseConfig
                    env = DatabaseConfig.get_active_env()
                    # Convert to short form
...
def switch_env(env, status):
    ...
    if status:
        ...
        try:
            if get_db_config:
                ...
                click.echo(f"Centralized configuration active environment: {db_config.get_active_env()}")
                
                # Display database URLs
                click.echo("Database URLs:")
                for env_name in ['development', 'testing', 'production']:
                    url = db_config.get_database_url_for_env(env_name)
                    # Mask password
                    url = mask_db_url(url)
    ...
    try:
        if get_db_config:
            ...
            click.echo(f"Active environment: {db_config.get_active_env()}")
            ...
            try:
                ...
                try:
                    ...
                    masked_url = mask_db_url(db_url)
                    click.echo(f"Active database URL: {masked_url}")
                except ImportError:
                    click.echo("Could not access database service for URL information")
    except Exception as e:
