...
# python scripts/manage_db.py clean-migration-files --force
# Stamp to latest migration (head) python scripts/manage_db.py stamp-migrations
# Stamp to base (no migrations):  python scripts/manage_db.py stamp-migrations base
# Stamp to a specific revision:   python scripts/manage_db.py stamp-migrations <revision_hash>
# Force stamp without confirmation:   python scripts/manage_db.py stamp-migrations --force
...
#         logger.info(f"Detected {len(changes.get('changes', []))} schema changes")
#         return changes
...
#     """Alias for sync-dev-schema"""
#     return sync_dev_schema(force=False)
...
def sync_models_to_db(force):
    ...
    try:
        ...
        if current_env != 'development':
            click.echo(f"Current environment is {current_env}. Schema sync only allowed in development.")
            if not force and not click.confirm("Do you want to continue anyway?", default=False):
                click.echo("Operation cancelled")
                return
    except ImportError:
        # Fallback check
        if os.environ.get('FLASK_ENV') not in (None, 'development'):
            click.echo("ERROR: Cannot use sync-models-to-db outside development environment")
            if not force and not click.confirm("Do you want to continue anyway?", default=False):
                click.echo("Operation cancelled")
                return
    ...
    with app.app_context():
        try:
            ...
            click.echo(f'FAILED: Database schema sync failed: {e}')
            sys.exit(1)
