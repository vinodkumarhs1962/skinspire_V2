-- =====================================================================
-- Patient Payment Receipts View - Many-to-Many Support
-- Based on ar_subledger table to show payment allocations to invoices
-- One row per payment-invoice allocation (M x N rows)
-- =====================================================================

-- Drop existing view
DROP VIEW IF EXISTS v_patient_payment_receipts CASCADE;

-- Create new view based on ar_subledger
...
SELECT
    ...
    -- === INVOICE REFERENCE (NULLABLE for multi-invoice payments) ===
    pd.invoice_id,  -- May be NULL for multi-invoice payments
    ih.invoice_number,
    ih.invoice_date,
    ih.invoice_type,

    -- === PATIENT INFORMATION ===
    COALESCE(ih.patient_id, pd.patient_id) AS patient_id,  -- Try invoice first, fall back to payment
    ...
    pd.upi_id,
    pd.reference_number,
    pd.receipt_number,  -- Add receipt number
    ...
    -- === BRANCH INFORMATION ===
    COALESCE(ih.branch_id, pd.branch_id) AS branch_id,  -- Try invoice first, fall back to payment
...
FROM
    payment_details pd
    LEFT JOIN invoice_header ih ON pd.invoice_id = ih.invoice_id  -- ✅ Changed to LEFT JOIN
    LEFT JOIN patients p ON COALESCE(ih.patient_id, pd.patient_id) = p.patient_id
...
'Patient payment receipts view with LEFT JOIN to invoice_header - supports many-to-many payments (payment → multiple invoices via ar_subledger)';
