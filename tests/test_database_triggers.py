...
# Run with: pytest tests/test_security/test_database_triggers.py -v
...
# - Removed legacy TestDatabaseTriggersLegacy class
...
class TestDatabaseTriggers:
    """Test suite for database trigger functionality with database service"""
    
    def test_timestamp_update_trigger(self, test_hospital):
        """
        Test that update_timestamp trigger automatically updates timestamps
        
        Verifies:
        - Updated records have their updated_at timestamp updated
        - Timestamp is actually changed when record is modified
        """
        logger.info("Testing timestamp update trigger")
        ...
        with get_db_session() as session:
            ...
            logger.info(f"Testing timestamp trigger on table: {table_name}")
            ...
            logger.info(f"Updating record with {update_column} = {sql_value}")
            ...
            logger.info(f"Timestamp updated from {original_timestamp} to {new_timestamp}")
    ...
    def test_password_hashing_trigger(self, test_hospital):
        ...
        Verifies:
        - Passwords are automatically hashed when stored in database
        - Hash format is correct (bcrypt)
        """
        logger.info("Testing password hashing trigger")
        ...
        with get_db_session() as session:
            ...
            logger.info(f"Inserting test user with ID: {test_user_id}")
    ...
    def test_user_tracking_trigger(self, test_hospital):
        """
        Test that user tracking trigger sets created_by and updated_by
        
        Verifies:
        - created_by field is set on record creation
        - updated_by field is set on record update
        """
        logger.info("Testing user tracking trigger")
        
        # Skip in unit test mode - requires database triggers
        if not integration_flag():
            pytest.skip("User tracking trigger test skipped in unit test mode")
        ...
        with get_db_session() as session:
            ...
            table_name = table_info[0]
            logger.info(f"Testing user tracking on table: {table_name}")
            ...
            logger.info(f"Inserting test record in {table_name}")
            ...
            # Reset session variable - this is important even with rollback
        ...
        logger.info("User tracking trigger test passed")
    
    def test_account_lockout_trigger(self, test_hospital):
        """
        ...
        - Account status or locked_until field is updated
        """
        logger.info("Testing account lockout trigger")
        
        # Skip in unit test mode - requires database triggers
        if not integration_flag():
            pytest.skip("Account lockout trigger test skipped in unit test mode")
        ...
        with get_db_session() as session:
            ...
            logger.info(f"Simulating {max_attempts} failed login attempts")
            ...
            """)).scalar():
                ...
                logger.info(f"Account status is now: {status}")
            ...
            """)).scalar():
                ...
                logger.info(f"Account locked until: {locked_until}")
        
        logger.info("Account lockout trigger test passed")