...
class TestAuthentication:
    ...
    def test_concurrent_sessions(self, client, test_user, session):
        ...
        tokens = []
        for _ in range(7):  # Intentionally more than typical limit
            response = client.post('/api/auth/login', json={
                'username': 'test_user',
                'password': 'test_password'
            })
        ...
        # Just verify that oldest session was invalidated
        oldest_valid_index = len(tokens) - 5  # Assuming limit is 5
        for i, token in enumerate(tokens):
            response = client.get('/api/auth/validate', headers={
                'Authorization': f'Bearer {token}'
            })
            if i < oldest_valid_index:
                assert response.status_code == 401, f"Old session {i} should be invalid"
            else:
                assert response.status_code == 200, f"Recent session {i} should be valid"
