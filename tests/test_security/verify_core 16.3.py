...
class SystemVerifier:
    ...
    def run_pytest(self, test_path: str) -> Dict:
        ...
        try:
            ...
            if "--cov" not in test_path:
                test_path = f"{test_path} --cov={test_path.split('/')[0]} --cov-report=term"
                logger.info(f"Added coverage reporting: {test_path}")
            ...
            if "--cov" in test_path:
                ...
                cov_parts = test_path.split("--cov=")[1].split()[0]
                cmd.extend(["--cov=" + cov_parts, "--cov-report=term"])
    ...
    def verify_authorization(self) -> Dict:
        ...
        # Try to get diagnostic information before test
        try:
            from app.services.database_service import get_db_session
            with get_db_session(read_only=True) as session:
            ...
            if self.integration_mode:
                try:
                    ...
                    with get_db_session(read_only=True) as session:
                        ...
                        from app.models import ModuleMaster
                        test_modules = session.query(ModuleMaster).filter(
                            ModuleMaster.module_name.like('test_module_%')
                        ).all()
                        if test_modules:
                            logger.warning(f"Found {len(test_modules)} test modules that weren't cleaned up:")
                            for module in test_modules:
                                logger.warning(f"  - {module.module_name} (ID: {module.module_id})")
                except Exception as e:
