# Testing Plan for Enhanced Database Management Features
...
1. **Create Test Environments**
   ...
   # Move SQL files to triggers directory (with copies in original location)
...
2. **Ensure Database Connectivity**
   ```bash
   # Verify database connections for dev/test environments
   python scripts/manage_db.py check_db
...
### Phase 1: Core Configuration Module Tests

1. **Test Configuration Module**
   ```bash
   # Create simple test script
   python -c "from app.config.db_config import DatabaseConfig; print(f'Active environment: {DatabaseConfig.get_active_env()}')"
   python -c "from app.config.db_config import DatabaseConfig; print(f'Database URL: {DatabaseConfig.get_database_url()}')"
   ```

2. **Test Environment Switching**
   ```bash
   # Switch environments and verify correct database URLs
   python scripts/switch_env.py dev
   python -c "from app.config.db_config import DatabaseConfig; print(DatabaseConfig.get_database_url())"
   
   python scripts/switch_env.py test
   python -c "from app.config.db_config import DatabaseConfig; print(DatabaseConfig.get_database_url())"
   ```

### Phase 2: Backup and Restore Tests
...
1. **Create Database Backup**
   ...
   python scripts/manage_db.py list_backups
   ```

2. **Modify Database and Restore**
   ```bash
   # Make a small change to the database
   python -c "from app.services.database_service import get_db_session; from sqlalchemy import text; with get_db_session() as session: session.execute(text('CREATE TABLE IF NOT EXISTS _test_backup_table (id SERIAL PRIMARY KEY, test_column VARCHAR(50))'))"
   
   # Verify the table exists
   python -c "from app.services.database_service import get_db_session; from sqlalchemy import text; with get_db_session() as session: result = session.execute(text(\"SELECT EXISTS(SELECT 1 FROM information_schema.tables WHERE table_name = '_test_backup_table')\")).scalar(); print(f'Table exists: {result}')"
   
   # Restore from backup
   python scripts/manage_db.py restore_backup
   
   # Verify the table no longer exists (rolled back)
   python -c "from app.services.database_service import get_db_session; from sqlalchemy import text; with get_db_session() as session: result = session.execute(text(\"SELECT EXISTS(SELECT 1 FROM information_schema.tables WHERE table_name = '_test_backup_table')\")).scalar(); print(f'Table exists: {result}')"
   ```

### Phase 3: Database Copy Tests

1. **Modify Development Database**
   ```bash
   # Switch to development environment
   python scripts/switch_env.py dev
   
   # Create a test table in development
   python -c "from app.services.database_service import get_db_session; from sqlalchemy import text; with get_db_session() as session: session.execute(text('CREATE TABLE IF NOT EXISTS _test_copy_table (id SERIAL PRIMARY KEY, env VARCHAR(50))'));with get_db_session() as session: session.execute(text(\"INSERT INTO _test_copy_table (env) VALUES ('dev')\"))"
   
   # Verify table exists with data
   python -c "from app.services.database_service import get_db_session; from sqlalchemy import text; with get_db_session() as session: result = session.execute(text(\"SELECT env FROM _test_copy_table\")).fetchall(); print(result)"
   ```
...
2. **Copy to Test Database**
   ...
   # Verify table exists with data in test database
   python -c "from app.services.database_service import get_db_session; from sqlalchemy import text; with get_db_session() as session: result = session.execute(text(\"SELECT env FROM _test_copy_table\")).fetchall(); print(result)"
   ```

3. **Test Schema-Only Copy**
   ```bash
   # Add more data to dev database
   python scripts/switch_env.py dev
   python -c "from app.services.database_service import get_db_session; from sqlalchemy import text; with get_db_session() as session: session.execute(text(\"INSERT INTO _test_copy_table (env) VALUES ('dev-more-data')\"))"
   
   # Copy only schema to test (should preserve existing data)
   python scripts/manage_db.py copy_db dev test --schema-only
   
   # Verify test database has original data
   python scripts/switch_env.py test
   python -c "from app.services.database_service import get_db_session; from sqlalchemy import text; with get_db_session() as session: result = session.execute(text(\"SELECT env FROM _test_copy_table\")).fetchall(); print(result)"
   ```

4. **Test Data-Only Copy**
   ```bash
   # Add a new column to test database
   python scripts/switch_env.py test
   python -c "from app.services.database_service import get_db_session; from sqlalchemy import text; with get_db_session() as session: session.execute(text('ALTER TABLE _test_copy_table ADD COLUMN test_only VARCHAR(50)'))"
   
   # Copy only data from dev to test (should preserve schema)
   python scripts/manage_db.py copy_db dev test --data-only
   
   # Verify new column still exists but data has been updated
   python -c "from app.services.database_service import get_db_session; from sqlalchemy import text; with get_db_session() as session: result = session.execute(text(\"SELECT * FROM _test_copy_table\")).fetchall(); print(result)"
   ```

### Phase 4: Migration Management Tests

1. **Create Migration**
   ```bash
   # Create a migration that adds a test table
   python scripts/manage_db.py migrate -m "add test migration table"
...
2. **Apply Migration**
   ```bash
   # If not already applied, apply migration
   flask db upgrade
   
   # Verify migration applied
   python -c "from app.services.database_service import get_db_session; from sqlalchemy import text; with get_db_session() as session: result = session.execute(text(\"SELECT EXISTS(SELECT 1 FROM information_schema.tables WHERE table_name = 'alembic_version')\")).scalar(); print(f'Alembic version table exists: {result}')"
   ```
...
### Phase 5: Trigger Management Tests
...
1. **Apply Triggers**
   ...
   python scripts/manage_db.py verify_triggers
   ```

2. **Test Trigger Fallback Paths**
   ```bash
   # Temporarily rename the SQL file in one location
   mv app/database/triggers/functions.sql app/database/triggers/functions.sql.bak
   
   # Apply triggers (should use original location as fallback)
   python scripts/manage_db.py apply_triggers
   
   # Restore the file
   mv app/database/triggers/functions.sql.bak app/database/triggers/functions.sql
   
   # Now try with both files renamed
   mv app/database/functions.sql app/database/functions.sql.bak
   mv app/database/triggers/functions.sql app/database/triggers/functions.sql.bak
   
   # Apply triggers (should fail with proper error)
   python scripts/manage_db.py apply_triggers || echo "Failed as expected"
   
   # Restore the files
   mv app/database/functions.sql.bak app/database/functions.sql
   mv app/database/triggers/functions.sql.bak app/database/triggers/functions.sql
   ```

### Phase 6: Backward Compatibility Tests

1. **Test All Original Script Functions**
   ```bash
   # Test all existing functionality
   python scripts/manage_db.py check_db
   python scripts/manage_db.py apply_base_triggers
   python scripts/manage_db.py apply_all_schema_triggers
   python scripts/switch_env.py dev
   
   # Use any existing custom scripts
   for script in scripts/*.py; do
     if [[ $script != *"manage_db.py"* ]]; then
       echo "Testing $script"
       python $script --help || echo "No help available"
     fi
   done
   ```
...
```bash
# Remove test tables
python -c "from app.services.database_service import get_db_session; from sqlalchemy import text; with get_db_session() as session: session.execute(text('DROP TABLE IF EXISTS _test_backup_table, _test_copy_table'))"

# Optionally remove test backups
# find backups -name "*test*" -delete
```
...
1. **Full System Test**
   ...
   python scripts/manage_db.py reset_and_init
...
The manual testing steps above can be automated with an enhanced testing script that provides comprehensive coverage of all database management features. The script also ensures that your development and test environments are preserved in their original state after testing.

### Key Features of the Enhanced Script

1. **Comprehensive Test Coverage**
   - Tests all database management features systematically
   - Includes testing for configuration, backup/restore, database copying, migrations, and triggers
   - Tests backward compatibility with original scripts

2. **Environment Preservation**
   - Automatically backs up both development and test environments before testing
   - Restores environments to their original state after tests complete
   - Ensures your testing activities don't permanently alter your databases

3. **Detailed Status Reporting**
   - Provides clear pass/fail status for each test
   - Creates detailed logs including timestamps and error messages
   - Generates a final test summary report

4. **Flexible Test Selection**
   - Run specific test categories or the entire test suite
   - Control whether environment preservation is enabled
...
def main():
    ...
    try:
        ...
        if environment_backups and not args.no_env_preserve:
            ...
            # Always offer to restore even if tests failed
...
1. **Run all tests with environment preservation (default):**
   ```bash
...
2. **Run specific test categories:**
   ```bash
   python scripts/test_db_features.py --config --backup
   ```

3. **Run tests without environment preservation:**
   ```bash
   python scripts/test_db_features.py --no-env-preserve
   ```

4. **Run tests without cleanup:**
   ```bash
   python scripts/test_db_features.py --skip-cleanup
   ```

### Key Benefits of the Automated Testing Approach

1. **Comprehensive Coverage**: Tests all features of the database management system
2. **Environment Preservation**: Ensures both dev and test environments return to their original state
3. **Detailed Reporting**: Provides clear pass/fail status for all test cases
4. **Selective Testing**: Allows testing specific components as needed
5. **Error Recovery**: Handles interruptions gracefully and still restores environments